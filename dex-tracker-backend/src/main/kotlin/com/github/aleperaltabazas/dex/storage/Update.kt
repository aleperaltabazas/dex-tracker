...
class Update(
    ...
    fun where(filter: Document): Update {
        this.where = filter
        return this
    }

    fun set(key: String, value: Any?): Update {
        this.updates.add(Updates.set(key, convertToDocument(value)))
        return this
    }

    fun add(key: String, value: Any?): Update {
        this.updates.add(Updates.addToSet(key, convertToDocument(value)))
        return this
    }

    fun updateOne(): UpdateResult = coll
        .updateOne(where ?: Document(), Updates.combine(updates))

    fun updateMany(): UpdateResult = coll
